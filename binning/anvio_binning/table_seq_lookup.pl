...
# A script to extract sequences from a file based on their identifiers in a tab file
# initially written to extract fastQ reads based on BLAST hit table, but general purpose now
# assumes sequences in fastQ files are all single line, fastA seqs can be multiple lines
#
# Daan Speth 2017
...
SEQ: while (my $line = <READ_FILE>){
		...
		if ($file_type eq "fastA"){
			...
			if ($fc eq ">"){
				...
				@temp = split(" ", $line);				# split header on spaces only take the first part, in
 case you're not blasting reads but sequences with longer headers
				$temp_id = $temp[0];
